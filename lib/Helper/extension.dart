...
class NoInitialSpaceFormatter extends TextInputFormatter {
  @override
  TextEditingValue formatEditUpdate(
      TextEditingValue oldValue, TextEditingValue newValue) {
    // If the new value has an initial space, prevent it
...
Widget webImage(
    ...
    String placeholderImage = Assets.placeHolder,
...
Widget webSerImage(
    {required String imageUrl,
    String placeholderImage = Assets.service,
    double width = double.infinity,
    double height = double.infinity,
    BoxFit fit = BoxFit.fill}) {
  return imageUrl.isNotEmpty
...
class YourAncestorWidget extends InheritedWidget {
  final String data; // Add any data you want to share with descendants

  YourAncestorWidget({Key? key, required this.data, required Widget child})
  ...
  static YourAncestorWidget? of(BuildContext context) {
    return context.dependOnInheritedWidgetOfExactType<YourAncestorWidget>();
  }
}

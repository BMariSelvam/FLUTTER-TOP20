...
class BusinessOpenListViewBuilder extends StatefulWidget {
  const BusinessOpenListViewBuilder({Key? key}) : super(key: key);

  @override
  State<BusinessOpenListViewBuilder> createState() =>
      _BusinessOpenListViewBuilderState();
}

class _BusinessOpenListViewBuilderState
    extends State<BusinessOpenListViewBuilder> {
  //ListView Builder
  ...
  Widget build(BuildContext context) {
    return GetBuilder<BusinessAppointmentDetailController>(builder: (logic) {
      return (appointmentOpen.isNotEmpty)
          ? ListView.builder(
              padding: EdgeInsets.zero,
              itemCount: appointmentOpen.length,
                ...
                return GestureDetector(
                  onTap: () {
                    Get.toNamed(Routes.appointmentDetailScreen,
                        arguments: appointmentOpen[index]);
                  },
                  child: Card(
                    elevation: 0,
                          ...
                          SizedBox(
                            child: Row(
                              children: [
                                ...
                                Text(
                                  '${TimeUtils.toHHMM(dateString: appointmentOpen[index].appoinmentFromTime)} - ${TimeUtils.toHHMM(dateString: appointmentOpen[index].appoinmentToTime)}',
                                  style: TextStyle(
                                    fontFamily: MyFont.myFont,
                                  ),
                                ),
                                    ...
                                    children: [
                                      ...
                                      Flexible(
                                        child: Column(
                                          ...
                                          children: [
                                            Text(
                                              "${appointmentOpen[index].totailCoinsPaid} AED ",
                                              maxLines: 1,
                                              overflow: TextOverflow.ellipsis,
                                              style: TextStyle(
                                                fontFamily: MyFont.myFont,
